...
class TestStripeWebhookIdempotency:
    ...
    async def test_duplicate_event_handling(self, db_session: Session, test_user):
        ...
        with patch('app.services.credits.add_credits') as mock_add_credits:
    ...
    async def test_concurrent_event_processing(self, db_session: Session, test_user):
        ...
        async def process_event():
            processor = StripeEventProcessor(db_session)
            return await processor.process_event(event_data)
        ...
        assert mock_add_credits.call_count <= 1

        # Verify only one database record
        events = db_session.query(StripeEventLog).filter(
            StripeEventLog.stripe_event_id == "evt_test_concurrent"
        ).all()
        assert len(events) == 1
    ...
    async def test_webhook_endpoint_integration(self, test_client: TestClient, db_session: Session, test_user):
